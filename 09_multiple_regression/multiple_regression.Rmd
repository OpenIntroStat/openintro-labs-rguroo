...
numerical. The meaning of each variable can be found by bringing up the codebook before importing the dataset from the Repository.
...
```{r scatter-score-bty_avg, echo = FALSE, results = "asis"}
...
Add the line of the bet fit to your plot by checking the `LS Line` box:
    
```{r scatter-score-bty_avg-line, echo = FALSE, results = "asis"}
knitr::include_graphics("img/scatterplot1-2.png")
```


***Can't jitter in Rguroo!***
...
5.  Let's see if the apparent trend in the plot is something more than
    natural variation. Using Rguroo's `Linear Regression` module, fit a linear model to predict average
...
take a look at the relationship between one of these scores (f1_lower) and the average 
beauty score.

```{r bty-rel, echo = FALSE, results = "asis"}
knitr::include_graphics("img/scatterplot2-1.png")
```

***Not sure how to do this in Rguroo***
...
into the model. To do this, we have to start with the `m_bty` model, open the `Basics`
menu, click the `+` sign, and double-click `gender` to add it to the `Formula` box.

```{r scatter-score-bty_avg_gender, echo = FALSE, results = "asis"}
...
9.  What is the equation of the line corresponding to male professors? 
...
comes first alphabetically as a $0$. You can change the reference level of a categorical variable, which is the level that is coded as a 0, by opening the `Level Editor`, selecting the variable of interest, and dragging-and-dropping the level of interest to the top of the list, as shown below:

```{r scatter-score-bty_avg_gender 2, echo = FALSE, results = "asis"}
...
```{r m_full,  echo = FALSE, results = "asis"}
knitr::include_graphics("img/MLR-FULL.png")
```

Save this model and report as `m_full`.

12. Check your suspicions from the previous exercise. Include a screenshot of the model output
    in your response.

13. Interpret the coefficient associated with the ethnicity variable.

Open the `Basics` menu. Delete the variable with the highest p-value from the `Formula` box and re-fit the model.

14. Did the coefficients and significance of the other explanatory variables change?
