...
First, open Rguroo's data editor by selecting **Create New Dataset** in the **Data** toolbox. In the data editor, click  `Add Rows/Variables`  ![add rows/variables](../icon_images/add_column.png) and in the dialog that pops up, select `Variable Properties`. Select the *Var1* variable; change its `Name` to *Affects* and change its `Type` to `Nominal`. Next, select the *Var2* variable; change its `Name` to *Probability* and keep its `Type` as `Numerical`. Then, close the dialog, and in the first two rows of the *coin_outcomes* variable, type in the values of "yes" and "no" for *Affects* and their corresponding probability weights for *Probability*. Here, since we assume 62% of responses will be from people who think climate change impacts their community, the probability of "yes" should be 0.62 and the probability of "no" should be 0.38, as shown in the figure below. **Save** the dataset with the name *climate_change_affects*.
