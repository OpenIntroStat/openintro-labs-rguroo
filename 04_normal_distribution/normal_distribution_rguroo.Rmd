...
In Rguroo, this can be done by clicking ![Basics](../icon_images/basics2_Selected.png) and, in the `Smoothing` section, checking the `Normal` box. The screenshot below shows this for the *dairyqueen* dataset.
...
To draw a normal probability plot for each level of the factor $restaurant$, select **Mean Inference** $\rightarrow$ **One & Two Population**. In the dialog box that opens, select the *fastfood* `Dataset`, and check the box `Normal Probability Plot`. There are two options to choose your variable(s). The first option includes `Variable 1` and `Variable 2`. You would use this option if you want to make a normal probability plot for one or two numerical variables in your dataset without taking into account a factor variable. In our example, we want to draw a normal probability plot for the numerical variable *cal_fat* for the level *Dairy Queen* of the factor variable *restaurant*. So, choose the second option. As shown in the figure below, select `Variable` *cal_fat* and `By Factor` *restaurant*. In section `Population 1`, select the `Level` of the factor, namely  *Dairy Queen*, for which we want a normal probability plot. By clicking the `Preview` button ![eye](../icon_images/preview.png), we get a normal probability plot for the fat calories (variable *cal_fat*) for Dairy Queen restaurants. The resulting graph is shown below.
...
```{r normal_probability_plot, echo = FALSE, results = "asis", fig.align = "center", fig.cap = "*Normal probability plot for calories from fat for Dairy Queen menu items*", out.width="75%"}
knitr::include_graphics("img/normal_probability_plot.png")
```
...
probability plot where the points closely follow the red line shown.  Any deviations
from normality leads to deviations of the points from that line.
...
As you might expect, you'll need to go to the **Probability-Simulation** toolbox for this. But first, we need to know the sample size, mean, and standard deviation for the Dairy Queen data. To get these values, go to the **Data** toolbox and select **Functions** $\rightarrow$ **Summary Statistic**. Select the *fastfood* `Dataset`. Then, from the `Numerical` dropdown, select *cal_fat*, and from the `Factor 1` dropdown, select *restaurant*. There are a few selected default statistics, including the mean, standard deviation, and sample size (count). You can deselect what you don't need or leave the default statistics selected. Click the `Preview` button ![eye](../icon_images/preview.png) to see the result. Below we show the **Basic Summary Statistic** dialog box and the resulting output. We have highlighted the values that we need for Dairy Queen.

```{r summary_dialog, echo = FALSE, results = "asis", fig.align = "center", fig.cap = "*Computing summary data of the variable cal_fat by restaurant*", out.width="75%"}
knitr::include_graphics("img/summary_dialog.png")
```

```{r summary_data, echo = FALSE, results = "asis", fig.align = "center", fig.cap = "*Summary of cal_fat data for each restaurant*", out.width="85%"}
knitr::include_graphics("img/summary_data.png")
```

For the *cal_fat* variable of the Dairy Queen data, the sample size is 42, the sample mean is 260.48, and the standard deviation is 156.49. Therefore we should simulate 42 values from a normal distribution with mean 260.48 and standard deviation 156.49.
...
Even better than comparing the normal probability of the observed data to a single normal probability plot of simulated values is to compare it to a few plots of simulated values. To do so, an option is to repeat the steps described in this section a few times. Specifically, each time change the `Seed` for the random number generator, create a plot for the generated values, copy and paste the graph to your report, and then at the end, compare the plots. If you are happy with this approach, you can skip reading the next section.
...
5.  Using the same technique, determine whether or not the calories from the McDonald's
