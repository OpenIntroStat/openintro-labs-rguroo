...
First, open Rguroo's Data Editor by selecting **Create New Dataset** in the **Data** toolbox. By default, a blank dataset with 500 rows and 30 variables (labeled *Var1* to *Var30*) is created. Click  `Add Rows/Variables`  ![add rows/variables](../icon_images/add_column.png) and in the dialog that pops up, select `Variable Properties`. Select the *Var1* variable; change its `Name` to *coin_outcomes* and change its `Type` to `Nominal`, as shown below. Then, close the dialog, and in the first two rows of the *coin_outcomes* variable, type in the values of "heads" and "tails." Save this dataset with the name *Coin*, as shown below. The saved dataset *Coin* should now only contain one variable, *coin_outcomes*, and two rows; the empty rows and variables are automatically deleted when the dataset is saved. The variable *coin_outcomes* can be thought of as a hat with two slips of paper in it: 
