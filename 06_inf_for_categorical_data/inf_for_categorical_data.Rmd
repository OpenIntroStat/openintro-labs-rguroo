...
In this lab, you will delve into a sample from the Youth Risk Behavior Surveillance System (YRBSS) survey, which uses data from high schoolers to help discover health patterns. The 
dataset is called `yrbss`. Import it from the Rguroo Data Repository.
...
Remember that you can use Rguroo's `Subset` dialog to limit the dataset to just non-helmet
wearers:

```{r no helmet subset, echo = FALSE, results = "asis"}
knitr::include_graphics("img/subset1-1.png")
knitr::include_graphics("img/subset1-2.png")
```

Save the dataset as `no_helmet`.

It may be easier to calculate the proportion if you create a new variable
that specifies whether the individual has texted every day while driving over the 
past 30 days or not. Use the `Transform` dialog to create a new variable, `text_ind`, taking the value `ifelse(text_while_driving_30d == "30", "yes", "no")` in the center box:

```{r indicator-texting, echo = FALSE, results = "asis"}
knitr::include_graphics("img/transform-1.png")
```

Save the new dataset as `no_helmet2`.
...
hypothesis test. These can both be found in the `Proportion Inference - One Population` dialog under Analytics.

First, we have to set up the dialog to indicate the `no_helmet2` dataset. Then, we want to select text_ind as our `Factor` variable and look at "yes" as the `Success` level.

```{r proportion-inference-setup, echo = FALSE, results = "asis"}
knitr::include_graphics("img/OneProp1-1.png")
```

Note that when you select the success level, you have three options: yes, no, and NA. NA means that the data is missing, but Rguroo treats missing data as a separate level. To remove the missing values when calculating the proportion, click `Level Editor` to bring up the Factor Level Editor. Select text_ind as the variable, then drag-and-drop the NA level to the `Dropped Level` section:

```{r proportion-inference-FLE, echo = FALSE, results = "asis"}
knitr::include_graphics("img/OneProp1-FLE.png")
```

*** Mori: this section does a bootstrap CI. I do not remember if the new functionalities for one proportion include a bootstrap option. If not, suggest an alternative. ***
...
1.  Using the `no_helmet2` dataset and the `One Population Proportion Inference` dialog, calculate confidence intervals for two other 
...
*** Mori: is there a way to make this dataset in Rguroo? Right now my "hack" way is to use Random Generator to get 1000 data points and then use transform to do p and margin of error ***

Since sample size is irrelevant to this discussion, let's just set it to
some value ($n = 1000$) and use this value in the calculations:

```{r n-for-me-plot, eval = FALSE}
n <- 1000
```
