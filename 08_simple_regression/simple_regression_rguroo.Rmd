...
This dataset has **a lot** of variables, but we are only interested in a few. There are also nine years of data in this dataset, but we are only interested in 2016. We can use Rguroo's **Subset** function to obtain only the variables of interest for 2016.

1. Using the `Logical Expression` dialog, select only the rows that have *year* equal to 2016. `Preview` ![eye](../icon_images/preview.png) the resulting dataset and **Save** it as *hfi_2016*. How many rows and columns are in the new dataset?

To select columns, click ![Basics](../icon_images/basics2_selected.png) to reopen the **Data Subset** dialog. Then, click the `Select Columns` button and a new dialog will open. The variables we will use in this lab are *pf_score*, *pf_expression_control*, *hf_score*, and *countries*. Find these variables in the `Variables` list on the left and drag-and-drop them to the `Selected Variables` list on the right. The easiest strategy for finding each variable is to type the first few characters of the variable name in the `Search Variable` box above the `Variables` list on the left. Once your have selected all four variables, as shown in the screenshot below, click `Done`. `Preview` ![eye](../icon_images/preview.png) the output and click the **Save As** button to overwrite the *hfi_2016* dataset.

```{r subset1, echo= FALSE, results = "asis", fig.cap = "*Subsetting columns in Rguroo*", fig.align='center'}
knitr::include_graphics("img/hifi_2016.png")
```
