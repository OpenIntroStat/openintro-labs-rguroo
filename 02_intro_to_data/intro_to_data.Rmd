...
1.  Replicate the following plot. **Hint:** The plot shows only flights from American Airlines (AA), Delta Airlines (DL), and United Airlines (UA), and the points are colored based on the `Factor` variable `carrier`. Rather than using **Subset**, you can create the scatterplot first, then click ![Level Editor](../icon_images/level_editor2_Selected.png) and use the **Factor Level Editor** to drop the unused levels and change the colors. Once you replicate
