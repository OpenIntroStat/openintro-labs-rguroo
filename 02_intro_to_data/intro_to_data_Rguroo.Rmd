...
When you are done reading the information about the data, import the dataset to your Data section.
...
histogram. Since this is a plot, we need to open the **Plots** section, click **Create Plot**, and select **Histogram**.
...
#### Subsetting by using the Subset function

As noted earlier, another way to get a histogram of departure delays for LAX is to get a subset of the data that only includes data values with destination LAX, and then use that dataset in the **Histogram** function to plot the variable *dep_delay*. To do this, go to the **Data** section, select **Functions**, and then click on **Subset**. A dialog box titled **Data Subset** should appear.
...
You can also obtain a custom numerical summary of the departure delays for these flights by going to the **Data** section, clicking **Function** and then selecting **Summary Statistic**:
...
```{r mean-dep-delay-months-1, echo= FALSE, results = "asis", fig.align = "center", fig.cap = "*Variable Type Editor to modify variable types*", out.width="80%"}
knitr::include_graphics("img/variable-type-editor.png")
```

Click `Update` to make the changes, and click `Close` to close the dialog box. Now open a new **Summary Statistic** dialog, select the dataset *nycflights*, and *month* will now appear as a `Factor 1` option, as shown in the dialog below.
...
<span style="color: red;"> *** At this point there is a problem as Rguroo does not want to save the output. I have reported it to Shahram. Let's try in the next deployment***</span> 
...
1.  Replicate the following plot. **Hint:** The plot shows only flights from American Airlines (AA), Delta Airlines (DL), and United Airlines (UA), and the points are colored based on the `Factor` variable *carrier*. Rather than using **Subset**, you can click ![Level Edtior](../icon_images/level_editor2_Selected.png) and use the **Factor Level Editor** attached to the plot to drop the unused levels and change the colors. Once you replicate
